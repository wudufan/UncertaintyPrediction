...
valid_generator = model.data.Image2DGeneratorForUncertainty(
    ...
    norm=train_args['Data']['norm'], scale_y = train_args['Data']['scale_y'],
    flip=False)
...
snapshot_callback = model.callbacks.TensorboardSnapshotCallback(
    network.model, snapshot_writer, snapshots_x, snapshots_y, postprocess = postprocess,
...
validation_callback = model.callbacks.SaveValid2DCallback(
    network.model, valid_generator, os.path.join(output_dir, 'valid'), train_args['Train']['save_freq'], 
    postprocess = postprocess, norm_x = display_args['norm_x'], norm_y = display_args['norm_y']
)

#%%
network.model.fit(generator, epochs = train_args['Train']['epochs'][-1], shuffle = False, 
