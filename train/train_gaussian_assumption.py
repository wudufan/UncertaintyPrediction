...
generator = model.data.Image2DGenerator(
    manifest, train_args['IO']['src_datasets'], train_args['IO']['dst_datasets'], train_args['IO']['train'], **train_args['Data'])
valid_generator = model.data.Image2DGenerator(
    manifest, train_args['IO']['src_datasets'], train_args['IO']['dst_datasets'], train_args['IO']['valid'], 
    patch_size=(640, 640), num_patches_per_slice=1, num_slices_per_batch=1, shuffle=False, 
...
snapshot_callback = model.callbacks.TensorboardSnapshotCallback(
    final_model, snapshot_writer, snapshots_x, snapshots_y, postprocess = postprocess, save_preds_all_channels=True,
...
validation_callback = model.callbacks.SaveValid2DCallback(
    final_model, valid_generator, os.path.join(output_dir, 'valid'), train_args['Train']['save_freq'], 
    postprocess = postprocess, norm_x = display_args['norm_x'], norm_y = display_args['norm_y'], save_preds_all_channels=True
)

#%%
final_model.fit(generator, epochs = train_args['Train']['epochs'][-1], shuffle = False, 
