...
class Image2DGenerator(tf.keras.utils.Sequence):
    def __init__(
        ...
        @src_datasets: list of strings or tuples. Each elements give the path to the h5 file to extract x from.
            If the element is a tuple of strings, it will extract the same slices from each of the h5 files
        ...
        @dst_datasets: list of strings or tuples. Each elements give the path to the h5 file to extract y from.
            If the element is a tuple of strings, it will extract the same slices from each of the h5 files
            and concatenate along the channel dimension.
            If len(dst_datasets) == 1, all the src_datasets are mapped to the same dst_datasets.
            Otherwise it should be a one-to-one mapping.
    ...
    def __getitem__(self, index: int):
        ...
        islices = self.slice_indices[idataset][
            slice_batch * self.num_slices_per_batch:(slice_batch + 1) * self.num_slices_per_batch
        ]

        # reload nii images if needed
        if idataset != self._current_idataset:
            if self.verbose > 0:
                print('Loading dataset {0}'.format(idataset))
                for filename in self.src_datasets[idataset]:
                    print('x: {0}'.format(filename))
                for filename in self.dst_datasets[idataset]:
                    print('y: {0}'.format(filename), flush=True)
...
if __name__ == '__main__':
    ...
    # generator = Image2DGenerator(
    #     manifest,
    #     src_datasets,
    #     dst_datasets,
    #     tags,
    #     patch_size=(64, 64),
    #     num_patches_per_slice=100,
    #     num_slices_per_batch=1
    # )
    # x, y = generator[0]

    src_datasets = [(
        '/home/dwu/trainData/deep_denoiser_ensemble/data/mayo_2d_3_layer_mean/dose_rate_4.h5',
        '/home/dwu/trainData/uncertainty_prediction/denoising_results/'
        'mayo_2d_3_layer_mean/l2_depth_3/dose_rate_4/dose_rate_4.h5'
    )]
    dst_datasets = [(
        '/home/dwu/trainData/deep_denoiser_ensemble/data/mayo_2d_3_layer_mean/dose_rate_1.h5',
        '/home/dwu/trainData/uncertainty_prediction/denoising_results/'
        'mayo_2d_3_layer_mean/l2_depth_3/dose_rate_4/dose_rate_4.h5'
    )]
    generator = Image2DGeneratorForUncertainty(
        manifest,
        src_datasets,
        dst_datasets,
        tags,
