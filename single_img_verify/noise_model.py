...
class ImageNoiseModel:
    ...
    def posterior_pdf(self, y, nsamples=200, sample_width=6):
        '''
        construct the posterior pdf p(x|y) for each pixel,
        from x0-sample_width*std_x0 to x0+sample_width*std_x0 with nsamples
        '''
        ...
        prob = 1 / std_x * np.exp(
            -(pdf_x - y[..., np.newaxis])**2 / (2 * std_x**2)
            - (pdf_x - self.x0[..., np.newaxis])**2 / (2 * std_x0[..., np.newaxis]**2)
        )
...
if __name__ == '__main__':
    ...
    plt.subplot(131)
    plt.imshow(x0, 'gray', vmin=0.84, vmax=1.24)
    plt.subplot(132)
    plt.imshow(x, 'gray', vmin=0.84, vmax=1.24)
    plt.subplot(133)
    plt.imshow(y[0], 'gray', vmin=0.84, vmax=1.24)
    ...
    plt.subplot(221)
    plt.imshow(post_mean[128:-128, 128:-128], 'gray', vmin=0.84, vmax=1.24)
    plt.subplot(222)
    plt.imshow(error_x0[128:-128, 128:-128], 'gray', vmin=0, vmax=0.1)

    if noise_model.std_x_use_x0:
        th_mean, th_std = noise_model.posterior_mean_and_std_analytical(y[0])
        plt.subplot(223)
        plt.imshow(th_mean[128:-128, 128:-128], 'gray', vmin=0.84, vmax=1.24)
        plt.subplot(224)
        plt.imshow(th_std[128:-128, 128:-128], 'gray', vmin=0.035, vmax=0.05)
